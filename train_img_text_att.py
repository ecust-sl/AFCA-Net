...
def train(data_loader, test_loader, val_loader, model, optimizer, scheduler, total_epochs, save_interval, save_folder, sets,
    ...
    swanlab.init(
        project="Med3D-KF-5",
        name=f"0911_fold_{num_KF}_2_3_loss_fusion_att",
        config={
    ...
    for epoch in range(total_epochs):
        ...
        for batch_id, batch_data in enumerate(data_loader):
            ...
            # fused_feat = volumes
            # print('shape================', volumes.shape)
            logits_2, logits_3 = model(volumes)
        ...
        root_path = '/extra/shilei/Medical-Result/Train'
        train_file_name = 'k_' + str(num_KF) + 'epoch' + '_' + str(epoch) + "_train.csv"
        train_path = os.path.join(root_path, train_file_name)
        # results_df = pd.DataFrame({
        #     'binary_prediction': pred_labels,
        #     'binary_probability': all_probs,
        #     'three_class_prediction': pred_labels_3,
        #     'class_0_probability': all_probs_3[:, 0],  # 第一类概率
        #     'class_1_probability': all_probs_3[:, 1],  # 第二类概率
        #     'class_2_probability': all_probs_3[:, 2]   # 第三类概率
        ...
        with torch.no_grad():
            for batch_id, batch_data in enumerate(test_loader):
                ...
                logits_2, logits_3 = model(val_volumes)
            ...
            root_path = '/extra/shilei/Medical-Result/Test'
            test_file_name = 'k_' + str(num_KF) + 'epoch' + '_' + str(epoch) + "_test.csv"
            test_path = os.path.join(root_path, test_file_name)
            # test_results_df = pd.DataFrame({
            #     'binary_prediction': val_preds,
            #     'binary_probability': val_probs,
            #     'three_class_prediction': val_preds_3,
            #     'class_0_probability': val_probs_3[:, 0],  # 第一类概率
            #     'class_1_probability': val_probs_3[:, 1],  # 第二类概率
            #     'class_2_probability': val_probs_3[:, 2]   # 第三类概率
            ...
            with torch.no_grad():
                for batch_id, batch_data in enumerate(val_loader):
                    ...
                    if not sets.no_cuda:
                        ...
                        # Forward pass
                    # val_fused_feat = torch.cat([val_volumes, val_flair], dim=1)
                    logits_2, logits_3 = model(out_val_volumes)
                ...
                # val_acc = accuracy_score(val_labels, val_preds)
                root_path = '/extra/shilei/Medical-Result/Val'
                val_file_name = 'k_' + str(num_KF) + 'epoch' + '_' + str(epoch) + "_val.csv"
                val_path = os.path.join(root_path, val_file_name)
                val_results_df = pd.DataFrame({
                    'binary_prediction': out_val_preds,
                    'binary_probability': out_val_probs,
                    'three_class_prediction': out_val_preds_3,
                    'class_0_probability': out_val_probs_3[:, 0],  # 第一类概率
                    'class_1_probability': out_val_probs_3[:, 1],  # 第二类概率
                    'class_2_probability': out_val_probs_3[:, 2]   # 第三类概率
...
if __name__ == '__main__':
    ...
    for i in range(1, 6):
        ...
        sets.gpu_id = 2
        sets.n_epochs = 500
        sets.save_path = "/extra/shilei/tongji_newdata/Med3D/09-11-best/"
        sets.model = 'resnet_attention'
        sets.pretrain_path = '/extra/shilei/tongji_newdata/resnet_34_23dataset.pth'
        # model_best_path = '/extra/shilei/tongji_newdata/Med3D/2-3-loss-fusion/fold_1_best.pth'
        sets.fusion_method = "ca"
        # getting model
